...
public class DateUtil {
    ...
    public static boolean leapYear(int year) {
        Boolean isLeap = Boolean.valueOf(false);
        if(year % 100 == 0 && year % 400 == 0 || year % 100 != 0 && year % 4 == 0) {
            isLeap = Boolean.valueOf(true);
        }

        return isLeap.booleanValue();
    }
    ...
    public static List<Integer> getMONTN(String start, String end) {
        ...
        int bing = Integer.valueOf(day_MONTH(start)).intValue();
        int ends = Integer.valueOf(day_MONTH(end)).intValue();
    ...
    public static int day_Week(String today) {
        try {
            Date e = (new SimpleDateFormat("yyyy-MM-dd")).parse(today);
    ...
    public static String getPreTimeSecond(String sj1, int second) throws Exception {
        ...
        try {
            Date e = format.parse(sj1);
            long Time = e.getTime() / 1000L + (long)second;
            e.setTime(Time * 1000L);
    ...
    public static boolean bigTime(String s1, String s2) {
        boolean sign = false;
        if(IfspDataVerifyUtil.isEmpty(s1)) {
            return false;
        } else if(IfspDataVerifyUtil.isEmpty(s2)) {
            return false;
        } else {
            if(s1.length() < 19) {
